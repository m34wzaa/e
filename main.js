...
import { makeObject } from './utils.js?v=13';
// make object, call function = pos, rot, id, name, corner, mat

const scene = new THREE.Scene();
scene.background = new THREE.Color(0x222222)
const fog = new THREE.FogExp2(0x222222, 0.4)
scene.fog = fog
let dist = 30
...
raycaster.near = 0.01
raycaster.far = 5
const crosshair = new THREE.Vector2(0, 0);const playerGeo = new THREE.BoxGeometry(0.8, 1.8, 0.8)
...
const lights = [rooms[0][2], rooms[0][9]]
const pointLights = lights.map(l => l.userData.pointLight);
...
function anim() {
    ...
    raycaster.setFromCamera(crosshair, cam);
    const hits = raycaster.intersectObject(rooms[0][5], true);
    intersects = hits.length > 0;
    pointLights.forEach(pl => {
        pl.visible = on === 1;
        pl.intensity = on === 1 ? 5 : 0;
    });
    if (timer > 0) timer--;
    pitch = Math.max(-lim, Math.min(lim, pitch));
